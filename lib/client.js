...
//  2. Q-version pet: procedural chibi drawn from the palette (always works),
//     plus an optional AI-generated pet via the Host /api/dsh-image-skin
//     proxy. The pet floats in the shell.overlay layer, draggable, with idle
//     bounce and click pulse.
//
// State persists in localStorage (per browser origin), so the skin and pet
// survive restarts without any host storage.
...
window.__ModuleLoader__.load({ id: 'dsh-image-skin', factory: (require) => {
  ...
  const PET_STYLE_ID = 'dsh-image-skin-style-pet'
  ...
  const DEFAULT_STATE = {
    ...
    pet: { enabled: false, size: 150, pos: { x: 24, y: 24 }, aiImage: null, useAi: false },
    ai: {
  ...
  // ------------------------------------------------------------------ pet drawing (procedural chibi)
  function drawPetImage(pal, size) {
    const S = Math.max(48, size)
    const canvas = document.createElement('canvas')
    canvas.width = S; canvas.height = S
    const g = canvas.getContext('2d')
    const dom = pal.dominant, sec = pal.secondary, acc = pal.accent, lit = pal.light
    const stroke = shade(dom, -0.38)
    // ground shadow
    g.fillStyle = 'rgba(0,0,0,0.12)'
    g.beginPath(); g.ellipse(S / 2, S * 0.90, S * 0.26, S * 0.045, 0, 0, Math.PI * 2); g.fill()
    // ears
    for (const ex of [S * 0.34, S * 0.66]) {
      g.fillStyle = sec
      g.beginPath(); g.arc(ex, S * 0.29, S * 0.12, 0, Math.PI * 2); g.fill()
      g.fillStyle = lit
      g.beginPath(); g.arc(ex, S * 0.30, S * 0.065, 0, Math.PI * 2); g.fill()
    }
    // body
    g.fillStyle = dom
    g.beginPath(); g.ellipse(S / 2, S * 0.52, S * 0.34, S * 0.31, 0, 0, Math.PI * 2); g.fill()
    g.lineWidth = Math.max(1.5, S * 0.018)
    g.strokeStyle = stroke
    g.beginPath(); g.ellipse(S / 2, S * 0.52, S * 0.34, S * 0.31, 0, 0, Math.PI * 2); g.stroke()
    // belly
    g.fillStyle = hexToRgba(lit, 0.75)
    g.beginPath(); g.ellipse(S / 2, S * 0.60, S * 0.17, S * 0.13, 0, 0, Math.PI * 2); g.fill()
    // eyes
    for (const ex of [S * 0.42, S * 0.58]) {
      g.fillStyle = '#ffffff'
      g.beginPath(); g.ellipse(ex, S * 0.45, S * 0.062, S * 0.080, 0, 0, Math.PI * 2); g.fill()
      g.fillStyle = '#26222e'
      g.beginPath(); g.arc(ex, S * 0.468, S * 0.033, 0, Math.PI * 2); g.fill()
      g.fillStyle = '#ffffff'
      g.beginPath(); g.arc(ex - S * 0.012, S * 0.450, S * 0.011, 0, Math.PI * 2); g.fill()
    }
    // blush
    g.fillStyle = hexToRgba(acc, 0.40)
    for (const ex of [S * 0.305, S * 0.695]) {
      g.beginPath(); g.ellipse(ex, S * 0.545, S * 0.052, S * 0.026, 0, 0, Math.PI * 2); g.fill()
    }
    // smile
    g.strokeStyle = stroke
    g.lineWidth = Math.max(1.2, S * 0.014)
    g.lineCap = 'round'
    g.beginPath()
    g.moveTo(S * 0.46, S * 0.545)
    g.quadraticCurveTo(S / 2, S * 0.585, S * 0.54, S * 0.545)
    g.stroke()
    // sparkle
    g.fillStyle = acc
    g.beginPath(); g.arc(S * 0.80, S * 0.18, S * 0.022, 0, Math.PI * 2); g.fill()
    g.beginPath(); g.arc(S * 0.755, S * 0.225, S * 0.014, 0, Math.PI * 2); g.fill()
  ...
  function PetOverlay() {
    ...
    const [pos, setPos] = useState((loadState() || DEFAULT_STATE).pet.pos || { x: 24, y: 24 })
    ...
    useEffect(() => {
      const fn = () => {
        const st = loadState()
        if (st) { setState(st); setPos(st.pet.pos || { x: 24, y: 24 }) }
      }
      window.addEventListener(EVT, fn)
      return () => window.removeEventListener(EVT, fn)
    }, [])

    if (!state.pet.enabled) return null
    const pal = state.palette || { dominant: '#7c9bd8', secondary: '#5a6fb8', accent: '#ff9ecb', light: '#eaf2ff', dark: '#1d2740' }
    const size = Math.max(48, state.pet.size || 150)
    const src = state.pet.aiImage && state.pet.useAi ? state.pet.aiImage : drawPetImage(pal, 220)

    const onPointerDown = (e) => {
      try { e.currentTarget.setPointerCapture(e.pointerId) } catch {}
      dragRef.current = { sx: e.clientX, sy: e.clientY, r: posRef.current.x, b: posRef.current.y }
      setDragging(true)
    }
    const onPointerMove = (e) => {
      const d = dragRef.current
      if (!d) return
      const maxR = Math.max(0, window.innerWidth - size - 8)
      const maxB = Math.max(0, window.innerHeight - size - 8)
      const r = Math.max(0, Math.min(maxR, d.r - (e.clientX - d.sx)))
      const b = Math.max(0, Math.min(maxB, d.b - (e.clientY - d.sy)))
      posRef.current = { x: r, y: b }
    ...
    const onPointerUp = () => {
      ...
      setDragging(false)
      const next = Object.assign({}, state, { pet: Object.assign({}, state.pet, { pos: posRef.current }) })
      saveState(next)
      notify()
    }
    const onClick = () => {
    ...
    const cls = 'dsh-skin-pet' + (dragging ? ' dragging' : '') + (pulse ? ' pulse' : '')
    return h('div', {
      className: cls,
      style: {
      ...
      title: '图片皮肤 Q 版宠物（拖动移动，点击互动）',
    }, h('img', {
      src, draggable: false, alt: 'Q版宠物',
      style: {
  ...
  // ------------------------------------------------------------------ AI generation
  async function aiGenerate(state) {
    if (!state.image) throw new Error('请先导入一张图片作为参考')
    const reference = await makeReference(await loadImage(state.original || state.image))
  ...
  function SkinPage() {
    ...
    const onAi = async () => {
      ...
      try {
        const image = await aiGenerate(state)
        const pet = Object.assign({}, state.pet, { aiImage: image, useAi: true, enabled: true })
        update({ pet })
        setAiMsg('生成成功，宠物已切换为 AI 形象')
      } catch (err) {
        setAiMsg('生成失败：' + err.message)
    ...
    return h('div', { style: { maxWidth: '720px', padding: '4px 8px', display: 'flex', flexDirection: 'column', gap: '10px' } },
      ...
      // ---- pet
      h('div', { style: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '12px', padding: '14px' } },
        row('Q 版宠物（开发中）', h('input', { type: 'checkbox', checked: !!state.pet.enabled, onChange: (e) => update({ pet: Object.assign({}, state.pet, { enabled: e.target.checked }) }) })),
        row('宠物大小 ' + state.pet.size + 'px', h('input', { type: 'range', min: 60, max: 240, value: state.pet.size || 150, style: { flex: 1 }, onChange: (e) => update({ pet: Object.assign({}, state.pet, { size: Number(e.target.value) }) }) })),
        h('div', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)', padding: '2px 0 6px 140px' } },
          '程序化 Q 版：由图片主色自动生成，圆滚滚的大眼宠物，出现在界面右下角，可直接拖动、点击互动。'),
        state.pet.aiImage ? row('使用 AI 形象', h('div', { style: { display: 'flex', alignItems: 'center', gap: '10px' } },
          h('input', { type: 'checkbox', checked: !!state.pet.useAi, onChange: (e) => update({ pet: Object.assign({}, state.pet, { useAi: e.target.checked }) }) }),
          h('img', { src: state.pet.aiImage, style: { width: '44px', height: '44px', objectFit: 'cover', borderRadius: '10px', border: '1px solid var(--dsw-alias-border-l2)' } }),
          h('button', { onClick: () => update({ pet: Object.assign({}, state.pet, { aiImage: null, useAi: false }) }), style: btnStyle('danger') }, '删除 AI 形象'),
        )) : null,
      ),

      // ---- AI generation
      h('div', { style: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '12px', padding: '14px' } },
        h('div', { style: { fontSize: '13px', fontWeight: 600, color: 'var(--dsw-alias-label-primary)', marginBottom: '6px' } }, 'AI 生成 Q 版形象（可选）'),
        row('API 地址', h('input', { type: 'text', value: state.ai.baseUrl, style: inputStyle(), onChange: (e) => update({ ai: Object.assign({}, state.ai, { baseUrl: e.target.value }) }) })),
        row('API Key', h('input', { type: 'password', value: state.ai.key, placeholder: '粘贴你的 API Key', style: inputStyle(), onChange: (e) => update({ ai: Object.assign({}, state.ai, { key: e.target.value }) }) })),
        row('模型', h('input', { type: 'text', value: state.ai.model, style: inputStyle(), onChange: (e) => update({ ai: Object.assign({}, state.ai, { model: e.target.value }) }) })),
        row('提示词', h('textarea', { value: state.ai.prompt, rows: 3, style: Object.assign({ flex: 1, resize: 'vertical' }, inputStyle()), onChange: (e) => update({ ai: Object.assign({}, state.ai, { prompt: e.target.value }) }) })),
        h('div', { style: { display: 'flex', alignItems: 'center', gap: '12px', padding: '6px 0 0 140px' } },
          h('button', { onClick: onAi, disabled: aiBusy || !state.image, style: btnStyle('primary') }, aiBusy ? '生成中…' : '生成 Q 版宠物'),
          h('span', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)' } }, aiMsg || '需要可用的生图 API Key（默认 MiniMax image-01 协议，兼容 OpenAI 风格接口）'),
        ),
      ),
  ...
  // ------------------------------------------------------------------ css
  const PET_CSS =
    '#dsh-image-skin-bg{position:fixed;inset:0;z-index:0;pointer-events:none;background-size:cover;background-position:center;background-repeat:no-repeat;transition:opacity .3s}' +
    '.dsh-skin-pet{position:fixed;z-index:2147483000;cursor:grab;user-select:none;pointer-events:auto;filter:drop-shadow(0 4px 10px rgba(0,0,0,.28));transition:transform .15s}' +
    '.dsh-skin-pet.dragging{cursor:grabbing;transform:scale(1.06)}' +
    '.dsh-skin-pet img{width:100%;height:100%;display:block;pointer-events:none}' +
    '.dsh-skin-pet.pulse{animation:dsh-skin-pulse .55s ease}' +
    '@keyframes dsh-skin-pulse{0%,100%{transform:scale(1)}40%{transform:scale(1.18)}0%{transform:scale(1)}}'

  // ------------------------------------------------------------------ apply
  function apply(ctx) {
    THEME = ctx.get('theme')
    // pet CSS (one style element owned by this plugin)
    ctx.effect(() => {
      let styleEl = document.getElementById(PET_STYLE_ID)
      if (!styleEl) {
        styleEl = document.createElement('style')
        styleEl.id = PET_STYLE_ID
        document.head.appendChild(styleEl)
      }
      styleEl.textContent = PET_CSS
      return () => { const el = document.getElementById(PET_STYLE_ID); if (el) el.remove() }
    }, 'dsh-image-skin-css')
    ...
    if (slots !== undefined) {
      ...
      slots.inject('shell.overlay', () => slots.register(
        { name: 'shell.overlay', id: 'dsh-image-skin-pet', order: 0 },
        PetOverlay,
      ))
    }
